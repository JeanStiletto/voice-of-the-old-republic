...
// acc::menus::crafting::DispatchRowCommit(panel, listBox, row):
...
// create/breakdown screens; resolved from the panel's ctor-bound member
// at drain time). The engine's own button handler then acts on the
// selected row — same select-then-confirm shape the K1 WorkbenchItems
