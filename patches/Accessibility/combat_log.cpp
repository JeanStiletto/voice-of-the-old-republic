// combat-log message parsing and the spoken result lines.
//
// Split out of combat.cpp by the Phase-1 structure pass (refactoring
// candidate 7). combat.cpp keeps combat-mode tracking (TickCombatMode /
// TickCombatLog and the IsCombatActive / IsPartyInCombat predicates);
// everything here is the acc::msg::Router rule set that turns the engine's
// localized combat-log lines into the mod's short spoken announcements.
//
// The two halves already lived in separate namespace blocks in the
// original file and share no state - the parser owns all of its own
// statics, and core_tick reaches the three tickers through combat.h. The
// move is verbatim, including this file's own extra include block below.
//
// Parsing is anchor-based against the localized strings in
// combat_strings.*; see project_combat_loc_en_phrase_mit_bug for why the
// anchors are fragile and must not be "tidied".
