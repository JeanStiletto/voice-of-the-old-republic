...
namespace ws = acc::spatial::wall_surfaces;

// --- Per-surface runtime state ----------------------------------------
//
// One slot per clustered surface. These are *runtime* state (fire
// timestamps, per-tick scratch) — not part of the clustering itself,
// which lives in spatial_wall_surfaces.{h,cpp}. Sized via the same
// constant so the arrays stay aligned with the surface-id space.
...
DWORD g_surface_last_cued_at[ws::kMaxWallSurfaces];

// --- Per-tick same-closest-point dedup tolerance ----------------------
//
// 5cm. T- and X-junction vertices have 3+ edges meeting at a single
// point; each edge clusters into its own surface (correctly, since
// they go off in different directions). When the player is closest to
// the shared vertex, every one of those surfaces independently
// reports that vertex as its closest point, and our K-cap fires
// several near-identical cues from the same world location. We
// collapse them at candidate-collection time: any surface whose
// closest point coincides with an already-pending candidate within
// this tolerance merges into that candidate (keeping the smaller
// distance).
...
SurfaceScratch g_surface_scratch[ws::kMaxWallSurfaces];
...
void OnAreaChange(void* area) {
    // Rebuild (or clear) the wall cache + surface clustering. The new
    // TU owns the wall arrays and emits its own area-change log line.
    ws::RebuildForArea(area);

    if (!area) return;

    for (int i = 0; i < ws::kMaxWallSurfaces; ++i) {
...
void CalibrateInRange(void* area, const Vector& referencePos,
    ...
    const acc::engine::WallEdge* walls    = ws::GetWallBuffer();
    const int*                   edgeSids = ws::GetEdgeSurfaceIdBuffer();
    int                          wallCount    = ws::GetWallCount();
    int                          surfaceCount = ws::GetSurfaceCount();
    ...
    float  surfBestDist[ws::kMaxWallSurfaces];
    Vector surfBestPt[ws::kMaxWallSurfaces];
    bool   surfHasData[ws::kMaxWallSurfaces];
    ...
    if (walls && edgeSids) {
        for (int i = 0; i < wallCount; ++i) {
            ...
            int sid = edgeSids[i];
            if (sid < 0 || sid >= surfaceCount) continue;
...
// --- Legacy public accessors — forward to wall_surfaces:: -------------
...
int GetEdgeSurfaceId(int edgeIdx) {
    ...
    const int* buf = ws::GetEdgeSurfaceIdBuffer();
    return buf ? buf[edgeIdx] : -1;
}

bool SegmentCrossesSurface(const Vector& a, const Vector& b,
                           Vector& outHitPoint) {
    return ws::SegmentCrossesSurface(a, b, outHitPoint);
}
