...
// ---------------------------------------------------------------------------
// Charakterblatt structured opener.
//
// The vanilla in-game character sheet is a 6-attribute grid + HP/FP/XP +
// alignment slider + 4 buttons. With our generic content fingerprint it
// reads as a pipe-separated number soup ("14 | 5 | 4 | 4 | 13 | 999/999 |
// 36/36 | 14 | 12 | 12 | 12 | 14 | +2 | +2 | +1 | +1 | +2 | +1 | ...") —
// the user can't tell which value is which stat. The struct DOES expose
// every label as a separate inline CSWGuiLabel at known offsets (per
// CSWGuiInGameCharacter SARIF dump), so we can pair value+modifier+name
// into a structured speech.
//
// This runs on first SubScreen-open of InGameCharacter, queued AFTER the
// "Charakterblatt" announcement that AnnounceNewSubScreens already speaks.
// All struct-offset constants verified against
// docs/llm-docs/re/k1_win_gog_swkotor.exe.xml CSWGuiInGameCharacter members.
// ---------------------------------------------------------------------------
...
// labelled it.
static constexpr size_t kCharSheetLblClass    = 0x02e4;  // class name "Soldat" (lbl_class1)
static constexpr size_t kCharSheetLblLevel    = 0x06a4;  // level number "1" (lbl_level1)
static constexpr size_t kCharSheetLblFort     = 0x0924;  // fortitude save val
static constexpr size_t kCharSheetLblRef      = 0x0a64;  // reflex save val
static constexpr size_t kCharSheetLblWill     = 0x0ba4;  // will save val
static constexpr size_t kCharSheetLblXpThresh = 0x11e4;  // next-level threshold ("3001")
static constexpr size_t kCharSheetLblXpCur    = 0x1464;  // current XP ("6000")
static constexpr size_t kCharSheetLblDefStat  = 0x15a4;  // defense val
static constexpr size_t kCharSheetLblHp       = 0x16e4;  // HP "999/999" (Lane: lbl_force_stat)
static constexpr size_t kCharSheetLblFp       = 0x1824;  // FP "36/36" (Lane: lbl_vitality_stat)
static constexpr size_t kCharSheetLblStr      = 0x1d24;  // "14"
static constexpr size_t kCharSheetLblStrMod   = 0x1fa4;  // "+2"
static constexpr size_t kCharSheetLblWis      = 0x20e4;
static constexpr size_t kCharSheetLblWisMod   = 0x2364;
static constexpr size_t kCharSheetLblCha      = 0x24a4;
static constexpr size_t kCharSheetLblChaMod   = 0x2724;
static constexpr size_t kCharSheetLblInt      = 0x2864;
static constexpr size_t kCharSheetLblIntMod   = 0x2ae4;
static constexpr size_t kCharSheetLblCon      = 0x2c24;
static constexpr size_t kCharSheetLblConMod   = 0x2ea4;
static constexpr size_t kCharSheetLblDex      = 0x2fe4;
static constexpr size_t kCharSheetLblDexMod   = 0x3264;

// Alignment slider — CSWGuiSlider @+0x55c4. cur/max at the standard slider
// offsets per engine_offsets.h.
static constexpr size_t kCharSheetSldAlign    = 0x55c4;
...
static void ReadCharSheetLabel(void* panel, size_t offset,
                               char* outBuf, size_t bufSize) {
    if (bufSize == 0) return;
    outBuf[0] = '\0';
...
static void MaybeAnnounceCharacterSheet(void* panel) {
    if (!panel) return;
    ...
    // Build the announce string. German default; uses pre-formatted
    // attribute modifiers ("+2") from the engine so we don't reimplement
    // the +N/-N formatting. Skips fields that read as empty rather than
    // emitting bare "Stärke ." sentences.
    char msg[1024];
    size_t off = 0;
    ...
    if (cls[0])    append("%s. ", cls);
    if (lvl[0])    append("Stufe %s. ", lvl);
    if (xpCur[0] && xpThresh[0]) {
        append("Erfahrung %s von %s. ", xpCur, xpThresh);
    }
    if (hp[0])     append("Lebenspunkte %s. ", hp);
    if (fp[0])     append("Machtpunkte %s. ", fp);
    if (str[0])    append("St\xE4rke %s%s%s. ",
                          str, strMod[0] ? ", " : "", strMod);
    if (dex[0])    append("Geschicklichkeit %s%s%s. ",
                          dex, dexMod[0] ? ", " : "", dexMod);
    if (con[0])    append("Verfassung %s%s%s. ",
                          con, conMod[0] ? ", " : "", conMod);
    if (intel[0])  append("Intelligenz %s%s%s. ",
                          intel, intMod[0] ? ", " : "", intMod);
    if (wis[0])    append("Weisheit %s%s%s. ",
                          wis, wisMod[0] ? ", " : "", wisMod);
    if (cha[0])    append("Charisma %s%s%s. ",
                          cha, chaMod[0] ? ", " : "", chaMod);
    if (alignMax > 0) {
        append("Gesinnung %u von %u.", alignCur, alignMax);
    }
