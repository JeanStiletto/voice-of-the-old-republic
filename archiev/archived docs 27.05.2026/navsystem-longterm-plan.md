...
- **Phase 6 — Map cycle + parity** (*closed 2026-05-18*; reshaped same day from original "Map markers & nice extras"). The map UI is now a self-sufficient surface for blind users: cycling keys work on it, every category sighted players see rendered (Door / Landmark / Transition / MapPin) is announceable + beaconable, all fog-of-war gated; the player can also drop their own markers on the map for navigation reference. Lay-offs:
  - **1a — Context-routed cycle.** `,/./Shift+,/Shift+./-/Shift+-/Ctrl+-/Alt+-/Enter` route to a separate map-context state singleton when `engine::HasActiveMapPanel()` is true. *Closed.*
  - **1b — MapPin category.** Quest-marker iteration over `CSWCArea.map_pins[]` (`+0x1c4` pointer-array). Map-pin focus uses Ctrl+- only (no UseObject path); Shift+- / Alt+- / Enter speak localized hints redirecting to Ctrl+-. *Closed.*
  - **2 — Map-state announcement keys.** AltGr in map context speaks map-frame compass + room name; uses `CSWSAreaMap::GetMapRotateCCWFromWorldOrientation @0x578ed0`. *Closed.*
  - **3 — Saved user markers (first slice).** Shift+N drops a `CSWCMapPin` at the cursor's world position via `engine_area::CreateMapPin` (replicating the engine's `operator_new(0x110)` → `CSWCMapPin::CSWCMapPin` → field writes → `CSWCArea::AddMapPin` pattern from `HandleServerToPlayerMapPinReferenceNumber @0x652d60`). User pins surface in the existing MapPin cycle + map cursor hover (both extended in-session: cycle fog-gate now exempts user pins via the high-bit reference-number tag; cursor scans `map_pins[]` in parallel with waypoints). *Closed.* **Two follow-ups deferred outside Phase 6:** (a) save/load persistence via per-player `CSWSScriptVarTable` writes (the K1 engine's own user-pin path; the original `CSWGlobalVariableTable.locations` assumption from the 2026-05-03 plan was wrong); (b) "speak active quest text" hotkey (re-scope of the original "quest-objective shortcut" — K1 has no engine quest→pin link, so the original "jump cursor to active quest pin" plan doesn't apply).
  - **Skipped — Party-member arrows.** Companions stay within ~1m of the player in 95% of normal play; surfacing them as cyclable would be high-noise / low-value. Solo Mode + scripted splits are covered by the existing in-world Tab leader-announce.
  **Exit criterion met:** cycle + announce + Ctrl+- beacon work for all map-rendered categories; map-state announce + saved-marker first slice both verified in-game.
...
- 2026-05-18 — **K1 user-pin storage is per-player ScriptVarTable, not `CSWGlobalVariableTable.locations`.** Ghidra decomp of `HandlePlayerToServerMapPinSetMapPinAt @0x524c70` showed the engine increments `NW_TOTAL_MAP_PINS` on the player creature's `CSWSScriptVarTable` (`CSWSCreature.field54_0x100`) and stores `NW_MAP_PIN_{NTRY,XPOS,YPOS,ZPOS,AREA}_{N}` per pin; persistence rides save/load via the player creature's serialised script vars. The 2026-05-03 plan reference to `CSWGlobalVariableTable.locations` was wrong. Phase 6 lay-off 3 first-slice ships in-area-only via `engine_area::CreateMapPin`; the ScriptVarTable write path is a deferred follow-up.
- 2026-05-18 — **K1 has no engine-side quest→map-pin link.** Original Phase 6 lay-off 3 "quest-objective shortcut" was specced against `CSWSPlayerJournalQuest` + `(plot_index, planet_id)` keys against `CSWCArea::GetMapPin @0x605ac0`. RE this session showed `GetMapPin(flags, subtype)` keys on the pin's `(+0x108, +0x10c)` pair, which the network handler sets to `(reference_number, 1)` from the player-side "place pin here" message. Journal entries (`SJournalEntry`) carry only text + tag + `plot_index/planet_id`, with no engine-side link to map pins. The "quest-shortcut" idea is permanently parked for K1; the re-scoped replacement ("speak active quest text" hotkey) is a separate future feature, not a Phase 6 closure dependency.
