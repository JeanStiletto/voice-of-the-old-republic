...
- **Phase 6 — Map cycle + parity.** *Closed 2026-05-18.* All four lay-offs verified in-game. Pillar 4 cycle keys (`,`/`.`/`Shift+,`/`Shift+.`/`-`/`Shift+-`/`Ctrl+-`/`Alt+-`/Enter) route to a separate map-context state singleton when `engine::HasActiveMapPanel()` is true. Map context surfaces **Door / Landmark / Transition / MapPin** — the four kinds the engine actually renders on the area map — fog-gated via `CSWSAreaMap::IsWorldPointExplored` for engine-placed entries; user-placed markers (high-bit reference numbers) bypass fog by design. Lay-offs 1a + 1b set up the cycle + MapPin category. Lay-off 2 added a map-frame AltGr branch ("{room}. Blick auf der Karte nach N° Grad, Nordosten.") via `engine_area::GetMapRotateCCWFromWorldOrientation`. Lay-off 3 first-slice added the Shift+N "drop saved marker at cursor" path: `engine_area::CreateMapPin` replicates the engine's `operator_new(0x110)` → `CSWCMapPin` ctor → field writes → `AddMapPin` pattern decoded from `HandleServerToPlayerMapPinReferenceNumber`; the new pin enters the existing MapPin cycle (lay-off 1b) and the map cursor's hover-pause (new `FindNearestMapPin` scan parallel to `FindNearestExploredMapNote`) so user pins are both cyclable and discoverable via WASD pan. Two follow-ups deferred to future work, not blocking Phase 6 closure: (a) save/load persistence for user markers (would require `CSWSScriptVarTable::Set{Int,Float,String,Object}` RE to write `NW_MAP_PIN_*_{N}` on the player creature — currently markers vanish on area transition); (b) "speak active quest text" hotkey (re-scope of the original "quest-objective shortcut" — K1 has no engine-side quest→pin link, journal entries are tag-keyed text only). Reshaped scope vs original 2026-05-03 plan: party-member arrows skipped (companions hug player 95% of normal play), saved-user-markers + quest-shortcut moved out of the original Phase 6 headline.
- **Phase 7 — User options UI.** Deferred per plan.

---

## Phase 6 — Map cycle + parity (closed 2026-05-18)
...
### Lay-off plan (all four closed)

1. **Lay-off 1a — Context-routed cycle (Door/Landmark/Transition).** *Closed 2026-05-18 (commit `a1d18f7`).*
2. **Lay-off 1b — MapPin category.** *Closed 2026-05-18.* Quest-marker iteration over `CSWCArea.map_pins[]`; `narrated_target.Slot` grew an `isMapPin` discriminator + frozen `pos`; map-pin activation-key branches.
3. **Lay-off 2 — Map-state announcement keys.** *Closed 2026-05-18 (this session, verified in-game).* AltGr on the map speaks map-frame yaw + sector + room name; world-frame branch on AltGr unchanged. Reuses `CSWSAreaMap::GetMapRotateCCWFromWorldOrientation @0x578ed0` + the three-tier room-name resolution (landmark → friendly name → "Raum N" synthetic).
4. **Lay-off 3 — Saved user markers (Shift+N) — first slice.** *Closed 2026-05-18 (this session, verified in-game — Shift+N drops a pin at the cursor's world position; pin appears in the MapPin cycle; map cursor speaks the pin's name on pan-back.)* Engine alloc pattern decoded + replicated; cycle + cursor surfaces extended; full UX vertical slice shipped.

**Deferred follow-ups** (out of Phase 6 scope; not blocking closure):

- **3-A: Save/load persistence for user markers.** Engine pins round-trip via per-player `CSWSScriptVarTable` strings (`NW_TOTAL_MAP_PINS` + `NW_MAP_PIN_{NTRY,XPOS,YPOS,ZPOS,AREA}_{N}` per pin — NOT `CSWGlobalVariableTable.locations` as the 2026-05-03 plan assumed). Our markers currently vanish on area transition. Replicating the four `CSWSScriptVarTable::Set*` writes would give parity with engine pins (engine itself re-creates from these vars on area-load). Pick up when there's user demand for persistence.
- **3-B: Speak-active-quest-text shortcut.** Re-scope of the original "quest-objective shortcut" — K1 has no engine-side quest→pin link. `CSWPartyTable.party_journal` (`CSWSJournal { SJournalEntry* data, size, capacity }`) is the read source; needs SJournalEntry "active" bit RE + a fresh hotkey allocation after the parked Phase 5 lay-off 5 registry rework.
...
**Lay-off 3 (first slice)** — Saved markers via Shift+N. *Verified in-game 2026-05-18.*

Engineering basis (RE confirmed this session via Ghidra-headless decompile):
- **Engine map-pin creation pattern** (decoded from `CSWCMessage::HandleServerToPlayerMapPinReferenceNumber @0x652d60`):
  1. `operator_new(0x110)` @0x43e1b0 — allocates the pin struct.
  2. `CSWCMapPin::CSWCMapPin()` @0x692540 — thiscall ctor, initialises vtable + zero-fields.
  3. Direct field writes: position@+0x24, enabled@+0xfc = 1, note_text CExoString @+0x100 via `CExoString::operator=(char*)` @0x5e5140, flags@+0x108 = reference number, subtype@+0x10c = 1.
  4. `CSWCArea::AddMapPin(this_, pin)` @0x606d90 — pointer-array append (decoded directly; engine grows the buffer via `CExoArrayList::Allocate`).
- **Lookup key**: `CSWCArea::GetMapPin(param_1, param_2) @0x605ac0` checks `pin->flags == param_1 && pin->subtype == param_2`. Engine pins use subtype=1 and a `NW_TOTAL_MAP_PINS`-derived monotonic counter (starting at 1). Our markers use subtype=1 with reference numbers in the high half (`0x80000000+seq`) to stay disjoint — single bit-test (`flags & 0x80000000`) discriminates user pins from engine pins downstream.
- **Lifetime**: pin is owned by `clientArea->map_pins[]` from AddMapPin onwards. Engine's `~CSWCMapPin` @0x6925b0 + `ClearAllMapPins` @0x606dd0 call `_free` matched to the alloc; same goes for note_text's c_string via `CExoString::operator=` writing through `operator_new` / `_free`.

Files touched:
- `engine_area.{h,cpp}` — `CreateMapPin(clientArea, pos, name, refNum, outPin)` wraps the four-step engine pattern under one SEH frame. `GetMapPinFlags(pin)` reader at +0x108 (used by cycle + cursor consumers to skip fog gate on user pins). Address constants: `kAddrOperatorNew`, `kAddrCSWCMapPinCtor`, `kAddrCExoStringAssignFromCString`, `kAddrCSWCAreaAddMapPin`.
- `map_user_markers.{h,cpp}` — new module. `PollWin32()` edge-detects Shift+N via the hotkey registry, gates on `HasActiveMapPanel + TryGetCursorWorldPosition`, composes an auto-name via the same three-tier room resolution lay-off 2 uses, calls `engine_area::CreateMapPin`, speaks confirmation. Per-area sequence counter resets on area change; reference numbers live at `kUserMarkerReferenceBase = 0x80000000` so they never collide with engine-counter pins.
- `cycle_state.cpp` — MapPin branch now reads `GetMapPinFlags` and bypasses the fog filter when bit 31 is set. Diagnostic line reports `user=N` count alongside `disabled` / `fog` / `kept`.
- `map_ui_cursor.cpp` — new `FindNearestMapPin` scans `CSWCArea.map_pins[]` in parallel with the existing waypoint scan. Both report squared pixel distance; closer hit wins. Engine pins fog-gated in the pin scan; user pins (high-bit flag) bypass fog. `CursorState` grew `pending_note_is_pin` / `last_spoken_is_pin` flags so the speak path dispatches `GetMapPinNoteText` vs `ReadWaypointMapNoteText`.
- `hotkeys.{h,cpp}` — `Action::SaveMarkerAtCursor` bound to Shift+N (Ctrl/Alt/AltGr forbidden). N picked because it has zero engine-vanilla binding — the semantically obvious Shift+M was tried first but the engine intercepted M to toggle the map panel regardless of Shift state, closing the very panel we wanted the marker on; Shift+Q also tried, opens the engine's Aufträge (Quests/Journal) menu. Provisional chord per the 2026-05-18 directive; moves when the parked Phase 5 lay-off 5 hotkey-registry rework lands.
- `core_tick.cpp` — `map_user_markers::PollWin32()` slot in the per-tick dispatcher.
- `strings.{h,cpp_de,cpp_en}` — `FmtSavedMarkerAutoNumber` ("Marke %d" / "Marker %d"), `FmtSavedMarkerAutoWithRoom` ("%s - Marke %d"), `FmtSavedMarkerPlaced`, `SavedMarkerFailed`.

Decisions captured:
- **Auto-name over user-entered name.** A TTS-driven name prompt is its own UX surface (editbox modal in map context, with state restore). Auto-naming via cursor's resolved room + sequence is information-dense and zero-keystroke; manual naming can land as a follow-up if the auto-naming proves insufficient.
- **Engine alloc/free over DLL-side bookkeeping.** Allocating via the engine's `operator_new` + adding to `map_pins[]` reuses the engine's MapPin pipeline end-to-end: visible to sighted players, picked up by our lay-off 1b cycle without changes, and the engine handles teardown on area unload. A DLL-side list would have required separate cycle plumbing, parallel beacon path, and would not have shown for sighted players.
- **User pins bypass fog gate.** First in-game test surfaced this — the user dropped at a cursor position that happened to land in an unrevealed cell; `IsWorldPointExplored` returned false and the cycle (which fog-filters all pins indiscriminately) excluded our marker. Fix: read the pin's flags and skip the gate when bit 31 is set. The cycle still fog-protects engine pins (no quest-spoiler leak) but the user's own pins are always findable. Same shape applied to the cursor scan.
- **Cursor pin scan added at first-test.** `map_ui_cursor` originally only scanned waypoints (CSWSWaypoint map_notes); pinning a pin and panning back to its spot would not have re-narrated it because `map_pins[]` was invisible to the cursor scan. Fixed in the same in-game iteration via `FindNearestMapPin` plus a kind-flag tag in the hover state so the speak path picks the right text accessor.
- **Hotkey chord landed on Shift+N after two retries.** Shift+M (semantically perfect — "map" semantics) failed because engine intercepts M unconditionally to close the map panel; Shift+Q failed because engine intercepts that as the Aufträge/Journal toggle. Shift+N is fully unbound in vanilla and not used by the mod — verified against `docs/controls-and-input.md`. Chord stays provisional until the parked hotkey-registry rework.
- **Persistence deferred to a follow-up slice.** Wiring the per-player `CSWSScriptVarTable` write path is meaningful new RE surface — it'd need address resolution for `CSWSScriptVarTable::SetInt/SetFloat/SetString/SetObject` plus reaching the player creature's table from our context. Worth a dedicated lay-off so the first-slice in-area shipping isn't gated on it.
- **Quest-objective shortcut dropped from this scope.** K1 has no engine-side quest→map-pin link (confirmed via decomp). A "speak active quest text" reinterpretation needs separate hotkey allocation + journal-state RE; defer until after the hotkey-registry rework.

**Lay-off 2** — Map-state announcement (AltGr on map). *Built 2026-05-18 — pending in-game verification.*

Files touched:
- `engine_area.{h,cpp}` — `GetMapRotateCCWFromWorldOrientation(areaMap, orientation, outDegCCW)` wraps `CSWSAreaMap::GetMapRotateCCWFromWorldOrientation @0x578ed0`. `__thiscall(this, Vector)` with the engine's float10 return treated as `double` (FPU ST(0) → fstp QWORD on the receiving side; the 80→64-bit precision loss is well below 1° announcement granularity). BYTES_PURGED=12 means the callee pops the by-value Vector args — `(void* __thiscall)(Vector)` typedef is correct as written.
- `strings.{h,cpp_de,cpp_en}` — `FmtMapStateOriented` (`"%s. Blick auf der Karte nach %d Grad, %s."` / `"%s. Facing %d degrees on the map, %s."`) and `FmtMapStateUnknownRoom` (room-less fallback). Sector word reuses `DirNorth..DirNorthwest`.
- `announce_degrees.cpp` — branch on `engine::HasActiveMapPanel()` after the player-position gate. Map branch: `GetPlayerFacing` → `GetMapRotateCCWFromWorldOrientation` → `EngineYawToCompass` → degrees+sector; room resolution chain matches `transitions::Tick` (landmark → friendly room → `Raum N` synthetic via `FmtTransitionRoomIndex`). Falls back to the world-frame announcement on any chain failure (engine not yet attached, faulting read) — never silent. World branch unchanged.

Decisions captured:
- **Engine map-rotate output is CCW-from-+X in map-local space**, same convention as engine yaw — so the existing `EngineYawToCompass` helper converts straight to compass-frame without a map-specific replacement.
- **Three-tier room resolution mirrors `transitions::Tick`**, not `map_ui_cursor::Tick`, because the player's position drives this announcement (cursor-driven announce is a separate surface). Landmark waypoint cache is already populated for the in-world transition path and shared via `transitions::GetLandmarkForRoom`.
- **Fall back to world-frame, never silent**, when the map context can't resolve facing or areaMap — the key already spoke world degrees in the non-map case, so keeping that audible avoids the "the key did nothing" failure mode (memory `feedback_never_silence_fallback_announcement`).
- **Cycle keys + AltGr coexist on the same context.** AltGr re-reads player state on demand; cycle keys narrate targets. No shared state; AltGr never interrupts an in-flight cycle announce because Tolk queues by default and the map-state line uses interrupt=true matching the existing world branch.

### Follow-ups parked outside Phase 6

Captured in the lay-off plan block above:

- **Save/load persistence for user markers** — replicate the per-player ScriptVarTable write path (`NW_TOTAL_MAP_PINS` + `NW_MAP_PIN_*_{N}` on `CSWSCreature.field54_0x100`); needs `CSWSScriptVarTable::Set{Int,Float,String,Object}` RE. Engine itself reads these vars on area-load and re-creates pins, so once the write path lands our markers reach full parity with vanilla user pins.
- **Speak-active-quest-text shortcut** — re-scope of the original "quest-objective shortcut" idea. K1 has no engine quest→pin link; the replacement candidate reads `CSWPartyTable.party_journal` (`CSWSJournal { SJournalEntry* data, size, capacity }`). SJournalEntry "active" bit offset is unverified (`PlaceHolder Class Structure` in Lane's `.h`); a quick Ghidra pass on `CSWSJournal::GetState` would surface it. Hotkey allocation gated on the parked Phase 5 lay-off 5 registry rework.

**Original 2026-05-03 plan note (preserved for context):** Saved user markers were originally specced against `CSWGlobalVariableTable.locations` (`CScriptLocation locations[100]`). That assumption was wrong — RE this session showed K1 persists user pins via per-player ScriptVarTable variables, not the global location table. Quest-objective shortcut was originally specced against `CSWSPlayerJournalQuest` with `(plot_index, planet_id)` → `CSWCArea::GetMapPin` keys. Also wrong — `GetMapPin` keys on `(reference_number, subtype=1)` set by the wire message, with no engine-side link to journal entries.
...
**Blocker recap (before opening 3a/3b):**
1. **Hotkey allocation.** The plan explicitly parks new chord allocations behind a Phase 5 lay-off 5 hotkey-registry rework — picking a chord now risks burning it for the quest shortcut and again later for the same job. Unblock when the registry pass lands, or accept a temporary chord with the explicit understanding it'll move.
2. **`SJournalEntry` → `GetMapPin` key mapping unverified.** Lane's `.h` lists `plot_index` @+0x34, `planet_id` @+0x38 as `undefined4` (`PlaceHolder Class Structure`). `CSWCArea::GetMapPin @0x605ac0` uses a `(flags, subtype)` pair internally (kMapPinFlagsOffset/kMapPinSubtypeOffset in engine_area.h, both noted as "used as a (flags, subtype) lookup pair") — same naming convention but different domain. Need a Ghidra-headless decompile of GetMapPin + the AddJournalQuestEntry path to confirm the keys overlap before writing the lookup.
3. **`CSWGlobalVariableTable.locations` API.** `CScriptLocation` layout + the read/write entry points aren't yet decoded. Save/load durability needs to be confirmed (do `locations[]` survive saves the way booleans/numbers do?).
