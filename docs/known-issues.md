...
- **Container Enter takes ALL items, not the highlighted one.** Enter on the loot panel fires `BTN_OK` (id=3, "Nehmen") which empties the entire chest into the player's inventory in one shot, even though we now drive `selection_index` ourselves on Up/Down (verified — per-row navigation announces correctly). Hypothesis: vanilla `BTN_OK` is "Take All" by engine design; per-item take in mouse-driven play happens via clicking on the listbox ROW, which fires the row's own onClick handler. Fix path: on Enter inside Container, `FireActivate` the row at `lb.controls[selection_index]` instead of `FireActivate(BTN_OK)`. Verify by checking whether row controls dispatch `kInputActivate` to a per-row take handler. The give-mode (`Ablegen`) path works correctly per item — places one selected item from inventory into the chest — so the take-all asymmetry is specifically on the take side.
