...
**Known defect (usable but not clean):** the line's mouse text is still read redundantly (keyboard hint + mouse sentence). Verified single-box (fired `box=` equals the panel `ContentChange`/`Menus.Chain` read), so it is NOT a two-popup mismatch — it is the multi-path announce problem below: the four announce paths (content-fingerprint, single-row listbox monitor, engine focus-announce, arrow-nav chain) don't agree on whether this box is "ours" because Surface-1 gates key on `tutorial.2da` mouse strrefs while the Trask popup's text is a **dialogue** strref. Fix is the unify work in the next entry — route all TutorialBox speech through one flag/identity authority instead of per-path text matching. FR/IT/ES dialogue hints and the reply-bracket lines (`48340`–`48343`) remain deferred.
