...
### Trask / Endar-Spire dialogue tutorial hints via a post-VO popup

The v0.6.0 tutorial-hint work converted the **silent** tutorial.2da popups to keyboard wording (shipped). The **voice-acted** Endar-Spire tutorial lines (Trask + the `end_pop*` windows) are a separate case: Trask speaks Basic with full VO, so his subtitle is (correctly) suppressed — we do NOT want the screen reader talking a diverging keyboard hint over his voice. Plan instead: keep his line suppressed during the VO, and **after the VO ends** — signalled by the next dialogue entry appearing or by control returning to the world (both already detectable in `dialog_speech.cpp` via the message-label change and the dialog-panel open/close poll; `CGuiInGame::HandleDialogEntry` @0x00631d80 is the precise per-line event if a hook is preferred) — surface the line's keyboard hint on a **self-created popup/menu** the user can read at leisure (mirror the synthetic-listbox pattern in `examine_view.cpp` / the mod-settings menu; a real engine MessageBox driven with custom text via `CSWGuiMessageBox::SetMessage` is the alternative). The hint content + strref→hint map are already authored and staged in `tutorial_hints.cpp` (`HintForDialogLine`, `kDialogHints`, the `TutTrask*` strings in `strings_*.cpp`); only the delivery surface + trigger remain. Reply-bracket lines (`48340`–`48343`, "[click this reply]") are deferred — they render on the reply-list path, not the entry path.
