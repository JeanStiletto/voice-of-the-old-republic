...
Items specific to the end-user installer at `installer/KotorAccessibilityInstaller/`.

- **Have to ship our bundled, modified KotorPatcher until the upstream PRs land.** The installer's `release.ps1` builds `KotorPatcher.dll` from our vendored `third_party/Kotor-Patch-Manager/` tree (which contains our local LEA/MOV, selective-POPAD ESP, and `consume_exit_address` EFLAGS/EAX fixes — see `docs/framework-changes-backup.patch`). Until those three fixes are upstreamed and a tagged release exists on Lane's GitHub, end users get our fork. Action: send PRs for each fix, then once they're merged and released, switch the installer to pull the official `KotorPatcher.dll` from Lane's release artifacts (or at minimum delete `docs/framework-changes-backup.patch`).

- **Shouldn't bundle Prism if it's online.** The installer currently embeds `prism.dll` (667 KB) from `third_party/prism-dist/x86/`. If Prism publishes a stable, addressable Windows x86 download (their own GitHub releases or a CDN), the installer should download it at install time the same way it downloads `Accessibility.kpatch`. That keeps the installer EXE smaller, picks up Prism updates without re-cutting our installer, and avoids us silently shipping a stale Prism with stale NVDA/JAWS/SAPI bridges. Action: check upstream Prism release cadence; if they ship Windows binaries with stable URLs, move Prism into `GitHubClient` alongside the kpatch download.

- **Mod install paths must be tested before automation.** The `docs/installer.md` survivor list (K1CP, Swoop Bike Upgrades, Thematic KOTOR Companions, Widescreen/FOV, JDR, Party Conversations on Ebon Hawk) needs end-to-end install-and-play verification against the German + English Steam installs before the installer auto-applies any of them. Currently only our `.kpatch` is wired into the install flow; the bundled-mods promise from the modding-info screen is descriptive, not yet enacted. Action: per-mod, drive a manual install, play through the relevant story beat with a screen reader, log result in `docs/installer.md`. Only after each survivor is green should it migrate into the installer's automated apply step.
