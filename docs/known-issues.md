...
### Radial (Shift+Enter) unreliable for keyboard-only play — cursor-coupled target

**Found:** 2026-06-01 beta log (`KotorAccessibility-Logs-20260601-142411.zip`, v0.2.0). User reported "can't unlock the security door." Log shows the Security row was offered and announced correctly, but "This object is locked." fired **71 times**; Security actually dispatched only once (and that open succeeded).

**Root cause:** the radial has no private copy of the menu — both `radial_menu::Tick()`'s disarm check and the Enter dispatch (`DispatchRowAction`) read the **live** engine `CSWGuiMainInterface` target-action menu (`ResolveTargetActionMenu`). We force-populate it for the keyboard-selected target via `SetMainInterfaceTarget` + `PopulateMenus` (engine_picker.cpp), but the engine re-derives that menu **every update** from the object under the **OS mouse cursor's** viewport ray. Our forced rows survive only while the engine's own cursor-ray happens to hit the same object. When it doesn't, the menu empties, `Tick()` calls `ForceDisarm("rows-empty")`, and the next Enter falls through to the default action ("Open" = 0x3f2), which on a locked door just says "This object is locked." Confirmed per-update (not mouse-move-gated): in the log the menu churned while the cursor was pinned at (408,346) and the player walked.

**Why it bites some users and not the maintainer:** purely environmental — the code is byte-identical across v0.2.0..HEAD. It depends on where the frozen OS cursor sits inside the 3D viewport. Fullscreen with a roughly-centered cursor tends to land on the faced target (works); **windowed** (`FullScreen=0`) maps the desktop cursor to an off-center / below-center viewport point that rarely coincides with the faced object (consistent failure). Mouse drift aggravates but isn't required.

**Fix direction (decouple from the cursor):** snapshot the rows (labels + action_ids) at arm time, stop polling the live struct in `Tick`, and re-install the target (`SetMainInterfaceTarget` + `PopulateMenus`) just-in-time on Enter before dispatching. (Alternative: anchor the OS cursor to the target's projected viewport coords on arm via `MoveMouseToPosition`.) Part of the broader resolution/fullscreen resilience initiative below.

### Crash on save — engine `ImageScale` divide-by-zero in the save-thumbnail path

**Found:** same 2026-06-01 bundle. Dump: `0xc0000094 INTEGER_DIVIDE_BY_ZERO` in `ImageScale`, call chain `CServerExoAppInternal::SaveGame` → `DoSaveGameScreenShot` → `AurSaveGameSnapshot` → `ImageScale`, writing `\saves\...\Screen.tga`.

**Root cause:** the engine generates the save-slot thumbnail from the framebuffer. The user's settings dump shows `Frame Buffer=0` (Frame Buffer Effects disabled) — the documented KOTOR trigger: with the framebuffer off the captured image is zero-sized and `ImageScale` divides by zero. Windowed mode can aggravate screenshot capture too.

**Fix directions:** (1) environment — installer writes sane `swkotor.ini` defaults (`Frame Buffer=1`, sensible resolution/fullscreen) so the default install can't hit it; (2) code — detour `ImageScale` (or `DoSaveGameScreenShot`) and bail/clamp when a dimension is 0 so saving never crashes regardless of the user's graphics settings (blank thumbnail is fine). Part of the resilience initiative below.

### Resolution / fullscreen / menu-offset resilience (umbrella)

The mod's promise is "no mouse, no screen," yet several code paths silently assume a particular screen geometry and break when the user changes resolution, runs windowed, or installs a community widescreen / HR-menus patch that moves GUI layouts. Known coupling points so far: the radial cursor-ray dependency (above), the save-thumbnail crash (above), in-world cursor parking, and any hardcoded hit-test offset compensations (e.g. the Options-panel tab-pitch shift). We deliberately avoided bundling the complex community resolution/menu patches for simplicity, but their changes — plus whatever resolution/windowed state a user lands in — must not be able to break core mod function. Needs: (a) an audit of every place we use screen-pixel coordinates, the OS cursor, or hardcoded offsets; (b) read geometry live from the engine instead of assuming; (c) installer-set sane graphics defaults; (d) a decision on auto-installing the widescreen + HR-menus patches and making our code robust to exactly the layout changes they introduce. See `installer.md` (Widescreen / HR-menus bundling) and the `project_radial_cursor_coupling` memory.
