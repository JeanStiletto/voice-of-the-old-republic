...
> **Resume here next session:** session 3 done — `ButtonToggle` / `LabelHilight` downcasts wired in, `CSWGuiListBox::SetActiveControl` hook works (chargen attribute panel firing as expected), TLK call attempted then reverted due to silent-patch regression. Speech policy locked at queued+dedup with always-on fallback. Five concrete issues mapped in the dated entry above. Tackle in priority order:
>
> 1. **Re-enable TLK lookup safely.** Wrap the `CTlkTable::GetSimpleString` call in a `__try`/`__except` block, bounds-check the strref against the engine's TLK entry count, and verify the `CExoString` struct size against the actual engine layout (Lane's SARIF says 8 bytes; double-check against a callsite's stack frame). Once stable, both `vtable=0073E5B8` (Label) and `vtable=0073E658` (Button) `src=none` cases should resolve to real text.
> 2. **Per-row listbox announcement.** Read `selection_index` at `+0x2c6` from the listbox `this` pointer, walk the `controls` CExoArrayList at `+0x29c`, and announce only the *currently selected* item's text. Falls back gracefully if the index is out of range.
> 3. **Capture per-screen input events.** Find `CSWGuiPanel::HandleInputEvent` (or whichever shared base actually dispatches arrow-key navigation across non-title screens) and hook it. Necessary to diagnose why arrow keys don't move sliders / advance options. Likely a SARIF query: `name == "HandleInputEvent"` filtered by `namespace ~ "CSWGui"`.
> 4. **Pointer-based dedup for placeholders** (`row 0` spam fix). Track last-seen `newControl*` per channel in addition to last-spoken text; suppress repeats only when the pointer matches.
>
> Use `dbgcapture` (in `tools/dbgcapture/`) for any future KotorPatcher diagnostics; the patch's own log file is `<install>\logs\patch-<utc>.log`. Latest reference log: `patch-20260430-014207.log`.
