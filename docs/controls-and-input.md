...
**Dead-keyboard watchdog.** The engine's DirectInput keyboard can die mid-session — a focus loss/regain makes it recreate its Render Window, the focus-gain path fires one `SetActive(0)->(1)` cycle, and when that single shot misses, no keystroke reaches the engine again. The mod's own hotkeys keep working throughout, because they are polled through `GetAsyncKeyState`, which is exactly what makes the failure so confusing to diagnose: mod speech and mod keys are alive while `Diag.ClientHIE` and `Menus.Input` are completely silent. `WatchDeadKeyboard` (`core_tick.cpp`) detects that asymmetry — an *engine-bound* key held per Win32 while `engine::LastEngineKeyEventMs()` has been stale for 1.5 s — and re-drives the acquire every 700 ms until a key lands. Only engine-bound keys count as evidence: the mod's own `,`/`.`/`-` legitimately never reach the engine. Logged as `EngineInput: dead-keyboard watchdog`. The cold-start twin (`RetryColdStartReacquire`) latches off after launch and does not cover this.

**Do not put a mod hotkey on Alt+letter — UNRESOLVED.** Alt+R blocked keyboard input for seconds at a time, recovering only when Alt was tapped again, with the patch log going *completely silent* in between (no `Diag.ClientHIE`, no `Menus.Input`): the presses never reached the engine, so nothing in the mod's input layer — including the modifier-shadow gates — could see or fix them. The obvious cause is the OS keyboard-menu mode: `DefWindowProc` answers Alt (alone, or Alt+letter as a menu mnemonic) with `WM_SYSCOMMAND`/`SC_KEYMENU` to open a menu bar the game does not have. `focus_guard.cpp`'s subclass proc now returns 0 for `SC_KEYMENU` / `SC_MOUSEMENU` (`SC_CLOSE`, i.e. Alt+F4, is left alone) — but a live round logged that swallow firing and the input still died, so **menu mode is not the whole story**; the engine's own `WM_SYSKEYDOWN` / DirectInput handling is the remaining unexamined suspect. `CopyLastSpoken` moved to Ctrl+R rather than chase it further. `PathfindFocusForce` (Alt+`-`/Alt+`/`) has never been reported as affected — punctuation does not go down the mnemonic path — but it is the registry's only remaining Alt binding and is worth re-testing if anyone touches this.

**Rising-edge model.** `Pressed()` reports an edge only when the binding's physical key was *up* last tick — not merely when the binding stopped matching. Two Actions sharing one key and differing only in modifiers (Ctrl+R copies the last spoken text, bare R repeats the dialog line) would otherwise both fire from one press: the bare Action's match is false while the modifier is down, so releasing the modifier with the key still held read as a fresh press. The key's raw state decides *when* a press happened; the modifier mask only decides *which* Action owns it.
...
- Ctrl+R — Copy the last spoken string to the Windows clipboard (`speech_clipboard.cpp`), then speak a short confirmation. Not gated on game state: it behaves identically in dialog, in the journal, in any menu and in the world, which is the whole point of the key. "Last spoken" is literal — the capture sits in `prism.cpp` at the three points where text reaches a backend (`Speak(wchar_t*)`, `Speak(char*)`, `SpeakUrgentImpl`), so a cue that fired after the line you wanted is what gets copied. The confirmation is excluded from the capture via `prism::SuppressNextCapture()`, so repeated presses re-copy the same text rather than "Copied to clipboard". Published as `CF_UNICODETEXT` (widened with `prism::GetSpeechCodepage()`) so umlauts and Cyrillic survive the paste; Windows synthesises `CF_TEXT` for ANSI consumers. The clipboard call runs on a short-lived worker thread and hands its result back to the next tick, which speaks the confirmation — the engine's main thread is an STA carrying the message loop and DirectInput dispatch, and `SetClipboardData` there wakes every clipboard listener on the system (NVDA, Windows 11 clipboard history). Same reasoning the `update_checker` note in `core_dllmain.cpp` gives for keeping WinHTTP off that thread. Same key family as bare R (re-speak the dialog line) and Shift+R (secondary radial). Alt and AltGr are both forbidden: AltGr on QWERTZ raises Alt plus a phantom Ctrl, which would otherwise make AltGr+R copy. Originally bound to Alt+R — see the Alt warning above for why it moved.
