...
### Open work — lay-off 5 (next session)

**Standalone description key.** Description currently rides Ctrl+-: the user always hears the route description when starting a beacon. The originally-planned "both modes off" fallback — a separate key that speaks the description WITHOUT arming beacon or autowalk — is not yet wired. Implementation is trivial since `guidance_description::Speak` is already a standalone entry point; just needs:

- A free hotkey (TBD; not Shift+-, not Ctrl+-; candidates: AltGr+- or `,`/`.` family).
- `cycle_input` handler that calls `ComputePath` + `description::Speak` and discards the path. No beacon, no autowalk.
- Edge case: if user re-presses while one mode is active, decision needed — speak alongside, or refuse? Probably speak alongside (description doesn't conflict with either mode's audio).

Lay-off 6 (map-UI cursor) and 7 (saved markers + quest-objective shortcut) remain pending and unchanged. Cross-area pathfinding stays deferred per the 2026-05-11 lock.
