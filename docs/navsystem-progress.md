...
- `patches/Accessibility/transitions.{h,cpp}` — new files. `Tick()` (per-tick area+room delta) + `AnnouncePreLoadDestination(exoStringPtr)` (called from the SetMoveToModuleString detour). Module-static dedup state for both paths.
- `patches/Accessibility/strings.{h, en, de}` — three new IDs (`FmtTransitionArea`, `FmtTransitionRoom`, `FmtTransitionLoading`).
