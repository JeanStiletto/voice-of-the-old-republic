...
10. **Octagonal direction-on-turn announcement** *(Phase 4 plan item, pulled forward 2026-05-04 — Build verified, awaiting in-game test).* Pillar 2 sub-feature C, locked **Default ON** in the long-term plan. Pulled into Phase 2 because the user reported "no clue if anything is working" while testing 9a/9b — they need confirmation the character is actually turning vs. only the camera. `turn_announce.{h,cpp}` (~110 lines) reads `acc::engine::GetPlayerYawDegrees`, converts engine frame (0°=East, CCW) → compass (0°=North, CW), buckets into 8 sectors of 45°, speaks the localised cardinal name on sector change with 5° hysteresis (per plan locked default). 8 new direction strings (`DirNorth` … `DirNorthwest`). Speech uses `interrupt=false` so direction doesn't talk over passive_narrate / cycle announcements. First-tick suppressed (don't announce just because DLL loaded).
...
Lay-off 9 fully scoped 2026-05-04. **9-probe + 9a closed and committed (`97b9715`).** **9b + turn-announcement built 2026-05-04** — both await in-game verification.

Next session: in-game test pass on the four newly-built features and the user's perception of "is it doing anything":
- **9a passive narration** — walk past doors / NPCs / containers, hear ambient cue + name on `LastTarget` change. Already validated for NPCs (Trask narrated in `patch-20260504-070033.log`); placeable / item / door cases still need a check.
- **9b interact hotkey (Enter)** — cycle to thing, press Enter, expect the engine to walk-to and open/talk/loot. Watch logs for `Interact: HandleMouseClickInWorld dispatched cleanly` and whether the player creature actually moves + interacts. Also serves as the autowalk-blocker side-channel test (parked Phase 2 follow-up).
- **Turn announcement** — Q/E should now speak "Norden / Osten / S\xFCden / Westen" + intercardinals on sector change. Validates that input is reaching the character.

Then pick:
- **Lay-off 7 — Pillar 2 transitions** (room-delta + area-load announce + room-cluster slice tightens cycle scope), or
- whatever falls out of the in-game test (likely a name-resolution polish, or unblocking of 9b's engine-side dispatch if HandleMouseClickInWorld doesn't move the player).
