...
- **Explicit Shift+- toggle-cancel** — *RE complete 2026-05-04 (DumpBytes pass).* Engine ships three named cancel primitives in Lane's DB:
  - **`CSWSCreature::RemoveAction(ulong action_id) @ 0x004f76c0`** — preferred. Takes the exact `action_id` we already capture from `AddMoveToPointAction` (the value `WalkTo` returns). 128-byte function; iterates the action queue (`creature + 0xfc`) looking for the matching id. Returns undefined4 (likely bool: found-and-removed). Direct-call wrapper, same pattern as `WalkTo` / `UseObject`. **Bytes at entry** (decoded from headless DumpBytes): `51 53 55 56 8b f1 8b 86 fc 00 00 00 8b 00 33 ed 85 c0 57 89 44 24 10 74 40 …` — push slot + callee-saved + load creature->action_table from +0xfc. We don't need to detour this; we just call it.
  - **`CSWSObject::ClearAllActions(int) @ 0x004ccd80`** — broader, clears the entire queue. 471 bytes (two ranges). Takes one `int` flag. Use when we want to clear everything (e.g. on combat enter); too coarse for "cancel just this autowalk".
  - **`CSWSPlayerLastUpdateObject::ClearActionQueue @ 0x005ca0d0`** — 810-byte internal-bookkeeping function, fired by the engine after queue mutations. Not for direct call.
  - **Implementation sketch**: add `bool acc::guidance::CancelAction(uint32_t action_id)` in `guidance_autowalk.{h,cpp}` wrapping `RemoveAction` (PFN typedef + SEH-guarded call, exactly like `UseObject`). In `cycle_input.cpp`, store `g_last_walk_action_id` from `WalkTo`'s return; second Shift+- press (debounce ~250 ms or simple "is autowalk in flight" flag) calls `CancelAction(g_last_walk_action_id)`, plays a brief acknowledgement cue, also calls `acc::engine::SetPlayerInputEnabled(true)` to re-enable manual control immediately rather than waiting for the 3s auto-restore. Speak `"Bewegung abgebrochen"` / `"Movement cancelled"` (new strings).
...
### Decision — defer the pre-load destination announce
...
- Function entry at `0x005edb50` is only 8 bytes; a clean 5-byte detour cut would corrupt the JMP-target encoded inside the thunk.
- Post-DumpBytes follow-up (2026-05-04, captured below in "Follow-ups parked from lay-off 7") found a better target anyway: **`CServerExoApp::SetMoveToModuleString @ 0x004aecd0`** captures the *destination module name* (e.g. `"endar_spire"`) directly — what the user actually wants to hear — while `AddMoveToModuleMovie` only carries the loading-movie name (e.g. `"load_endar"`). Hook target + cut bytes + pipeline-order verified; just needs the implementation lay-off.
...
Parked as a follow-up — the RE part is done; only the implementation remains. See "Follow-ups parked from lay-off 7" below for the actionable design.
...
- **Pre-load destination announce** — *RE complete 2026-05-04 (DumpBytes pass).* The deferred "Loading: …" announce. Two viable hook points decoded; `SetMoveToModuleString` is preferred:
  - **`CClientExoApp::AddMoveToModuleMovie @ 0x005edb50`** — entry is the 8-byte thunk `8b 49 04 e9 48 4c 01 00`: `mov ecx, [ecx+4]; jmp +0x14c48`. Tail-jumps to body at `0x006027a0`. The entry-point can NOT be detoured (5-byte cut would corrupt the JMP target). The body at `0x006027a0` is `8b 54 24 04 8d 81 d8 02 00 00 50 52 e8 …`: load `param_1 (CExoString*)` into EDX, `lea eax, [ecx+0x2d8]` for the movies-array, push, push, call `CExoArrayList<CExoString>::Add`. **Viable hook**: at `0x006027a4` (after the MOV, before the LEA) — 6-byte cut covering the LEA (register-relative, safe to relocate). At hook entry: ECX = `CClientExoAppInternal*`, EDX = `CExoString*` (the **movie name**, e.g. `"load_endar"` — NOT the destination module name).
  - **`CServerExoApp::SetMoveToModuleString @ 0x004aecd0`** — *preferred*. 14-byte thunk `8b 49 04 81 c1 84 00 01 00 e9 72 6f 13 00`: `mov ecx, [ecx+4]; add ecx, 0x10084; jmp CExoString::operator=`. Param is `CExoString*` = the **destination module name** (e.g. `"endar_spire"` / `"tar_m02ac"`) — exactly what the user wants to hear. **Viable hook**: at `0x004aecd0` (function entry) with a 9-byte cut covering MOV (3) + ADD (6) — both register/memory-relative, safe to relocate. ECX = `CServerExoApp*` (public facade) at hook entry; param_1 (`CExoString*`) at `[esp+4]`. Avoid the upstream stack-param feature (per `project_kpatchmanager_lea_bug.md`) by reading `[esp+4]` manually inside the handler, OR by hooking mid-function at `0x004aecd9` if a 5-byte register-only cut becomes available there. (Inner func at JMP-target `0x005e5c50` is `CExoString::operator=` — fired by hundreds of unrelated assignments, do NOT hook there.)
  - **Pipeline order** (per investigation §"Findings"): `SetMoveToModulePending(1)` → `SetMoveToModuleString(dest)` → `SetMoveToModuleStartWaypoint(wp)` → `AddMoveToModuleMovie(movie)`. All four fire BEFORE the loading screen plays, so a single hook at `SetMoveToModuleString` captures the destination in time for the announce to queue ahead of the load.
  - **Strings**: new `Id::FmtTransitionLoading` — `"Lade: %s"` / `"Loading: %s"`. Speech via `tolk::Speak(.., interrupt=false)` — the in-flight room-announce should finish first, then the loading announce queues behind it.
