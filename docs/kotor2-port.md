...
`docs/llm-docs/ingame-screens-reference.md` under k2_workbench_and_crafting
(is_active gate, the two cost functions, why the cost/stock labels must never
be quoted). Two cheap follow-ups nobody has looked at yet: whether the engine's
"not enough components" refusal and its Yes/No confirm box (create installs a
callback at `0x008D5FF0` for recipes expensive relative to skill) are reachable
and audible from the keyboard.
