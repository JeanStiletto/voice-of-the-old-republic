...
- **Phase 1 — Foundation.** `audio_bus.{h,cpp}` (+ optional `audio_listener.{h,cpp}` if engine default doesn't suffice); `engine_player.{h,cpp}`; `core_settings.{h,cpp}` (stub); curate the 12-cue audio vocabulary from existing engine resrefs (atmospheric pass) — fall back to authored WAVs in `Override/` only if curation proves too noisy. **Exit criterion:** test fixture can play a 3D positional cue at any world position with character-anchored listener. Commit. *(Revised 2026-05-03: `engine_area` moved out — split between Phase 2 and Phase 3 by consumer, see below. Sourcing approach revised from authored-into-Override to curated-from-existing-engine-resrefs.)*
