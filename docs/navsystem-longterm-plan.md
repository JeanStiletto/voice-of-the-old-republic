...
- **Phase 6 — Map cycle + parity** (*reshaped 2026-05-18 from original "Map markers & nice extras"; lay-offs 1a + 1b verified*). The map UI becomes a self-sufficient surface for blind users: cycling keys work on it, every category sighted players see rendered (Door / Landmark / Transition / quest-marker MapPin) is announceable + beaconable, all fog-of-war gated. Lay-offs:
  ...
  - **2 — Map-state announcement keys.** AltGr in map context speaks map-frame compass + room name; reuses `CSWSAreaMap::GetMapRotateCCWFromWorldOrientation @0x578ed0`. *Pending.*
  - **3 — Saved user markers + quest-objective shortcut.** Original Phase 6 headline, now last (nice-extra tier). Quest shortcut needs `CSWSPlayerJournalQuest` RE; saved markers use `CSWGlobalVariableTable.locations` per the 2026-05-03 batch lock. *Pending.* Planet picker + multi-area route prompt remain future.
  ...
  **Exit criterion:** cycle + announce + Ctrl+- beacon work for all map-rendered categories; lay-offs 2 + 3 ship the map-state announce + saved-marker / quest-shortcut nice-extras. Commit + release.
