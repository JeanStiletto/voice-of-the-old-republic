...
- **`kdev re <pattern> [--decompile] [--all] [--max-funcs N] [--out DIR] [--jq PATH]`** — reverse-engineer a whole bounded engine subsystem from Lane's Ghidra DB in one shot (the "decompile-first" playbook as a command). `<pattern>` is an Oniguruma regex matched against class/namespace names (and datatype names). It enumerates every function in the matching classes and dumps their struct layouts via the SARIF export (using `jq`), and with `--decompile` batch-decompiles the **logic-bearing** functions (skipping trivial `Set*`/`Get*`/ctor/dtor unless `--all`, capped at `--max-funcs`, default 40) through the Ghidra headless analyzer + `tools/ghidra-scripts/Decompile.java`. Emits `build/re/<slug>/<slug>-model.md` — a skeleton with a MODEL template (entry → input → state → action → effect), the full function inventory (trivial- and decompiled-marked), and struct layouts — plus `<slug>.decomp.txt` with the cleaned raw decompile. Curate the model doc into `docs/llm-docs/` once confirmed. Motivation: stop inferring engine models from noisy runtime measurements (which flip-flop); read the closed subsystem first (see `feedback_reconstruct_engine_structures_first` and `docs/llm-docs/turret-minigame-model.md`, the first model produced this way). Paths are configurable via the optional `[re]` section of `kdev.toml` (defaults match `docs/tools.md`); requires the SARIF export, `jq` on PATH, and a Ghidra install with the program imported.

- **`kdev dump-text [--out DIR] [--pid N]`** — captures the running game's **decrypted** image out of process memory, as a byte reference for `sigscan`. Exists because the Steam `swkotor.exe` is SteamStub-encrypted on disk (it carries a `.bind` section and its `.text` reads as ciphertext), so there is no offline source of known-good bytes at our hardcoded addresses. Reads the PE headers via `ReadProcessMemory`, then pulls each section at its `VirtualAddress` — one read per section, because an image has unmapped gaps between them and `ReadProcessMemory` is all-or-nothing. Writes `swkotor-image.bin` (the mapped image, **indexed by RVA, not file offset**) plus a `swkotor-image.json` sidecar with base/size/timestamp/section table. Finishes with a decryption sanity check: it measures the fraction of `.text` bytes that are common x86 opcodes (push/pop/mov/call/int3/ret) and fails if that sits near the ~2% a random-ciphertext buffer would score. The exe has no ASLR (`DllCharacteristics` 0, base 0x400000), so an address in the dump is the same address we hardcode. Needs one launch and does **not** need our DLL — SteamStub decrypts before the entry point runs.

- **`kdev sigscan [--reference FILE] [--target FILE] [--out DIR] [--emit-only]`** — rebases every hardcoded engine address onto a different build of `swkotor.exe`. Supporting another executable is a *relocation* problem, not a re-RE problem: the functions are byte-identical, the linker just moved them (measured +96..+512 bytes against the Allard Russian exe, varying per function and not monotonic). Two halves: harvest addresses, then locate them.
  - **Harvest** (`EngineAddresses`): the 25 `hooks.toml` detour sites (with their recorded `original_bytes`) plus **every hex literal in the image's VA range** anywhere in `patches/Accessibility/`, with `//` and `/* */` text stripped first — the sources quote addresses in prose constantly and those are not dependencies. Declaration shapes (`[static] [constexpr|const] [std::]uintptr_t NAME = 0x… | acc::addr::R(0x…)`, including the value-on-next-line form) are still matched, but only to recover a readable NAME for the report. Currently 303 references / 281 distinct addresses, 225 of them in `.text`; the rest are `.data`/`.rdata` and need no rebasing (verified unchanged between builds).

    **This used to be a declaration matcher and that was a bug** (found 2026-07-29). It keyed on `^(constexpr|const) uintptr_t NAME = 0x…;`, so an address written any other way was not merely unmatched — it was *invisible*: no signature, no table entry, and no appearance in the unresolved list. Twelve addresses were lost that way (`static constexpr`, `constexpr std::uintptr_t`, and inline `reinterpret_cast<PFN>(0x…)`), and each stayed at its reference value on the Allard build, calling a few hundred bytes into an unrelated function. Worse, the matcher could not see `= acc::addr::R(0x…);` either — the *correct* form — so regenerating the table would have silently dropped nearly every address already in it. Hence the inversion: everything in range counts unless a marker opts it out (`kdev-sigscan: ignore` on a line, `kdev-sigscan: ignore-file` in a file). A false positive is one noisy report line; a false negative is a crash on someone else's machine. `engine_rebase.cpp`'s `kXrefTable` carries the per-line marker, because its right-hand column holds *target*-build addresses that mean nothing in the reference.
  - **Signature** (`Signatures`): decodes forward with **Iced** and grows a masked pattern instruction-by-instruction until it is unique in `.text`. Wildcards exactly the operands that legitimately differ between builds — relative branch displacements (`call`/`jmp`/`jcc`, which shift whenever caller and callee moved apart) and absolute immediates or displacements pointing *into* `.text`. Displacements into `.data`/`.rdata` deliberately stay **concrete**: they are stable across builds and are the strongest discriminators available. Minimum 4 instructions (a 5-byte pattern matched 2 517 places when measured), cap 96 bytes.
  - **Resolve**: scans the target's `.text` and classifies each as `unique` / `ambiguous` / `not-found`. **It never picks a best candidate.** Most of these addresses are `reinterpret_cast` to function pointers and *called*, so a wrong answer executes another function under the wrong calling convention and crashes — unlike the 25 hook sites, which fail safe because the patcher verifies their bytes and refuses on mismatch. Addresses already mapped by hand in `engine_rebase.cpp`'s `kXrefTable` are reclassified `hand-resolved` and excluded from the failure count — they can never resolve by content (identical forwarding thunks, or a near-twin sharing the whole prologue), so counting them would leave the warning permanently on, and a warning that is always on is one nobody reads.
  - **Unresolved is an error, not a note.** Any `.text` address left `ambiguous` or `not-found` exits **7** with an explicit message. It used to exit 0, which meant the single condition the command exists to detect was also the one it stayed quiet about: a table could be regenerated, look healthy, and silently omit an address. Note that `acc::addr::R()` returns **0** for an address it cannot map, so an omission is a null dispatch at the call site unless guarded with `acc::addr::Ok()`.
  - **Built-in correctness check:** for the 25 detours, the generated signature is compared against the `original_bytes` already recorded in `hooks.toml`. Disagreement means the reference dump or the decoder is wrong, and every other signature is then suspect. Against a correct reference this must read 25/25.
  - Reference layout (memory dump vs on-disk file) is auto-detected from file length versus `SizeOfImage` and printed, because indexing a memory dump with file-offset logic silently reads the wrong bytes instead of failing.
  - Writes `build/re/sigscan/sigscan-report.{txt,json}` with the old/new address, delta, signature and any candidates.

  Self-test: pointing `--reference` and `--target` at the same exe must resolve every built signature at delta 0 (verified — 197/197).

  **Measured applicability (re-measured 2026-07-29 with the widened harvester).** Against the Allard Russian exe — a different build of the *same* BioWare source — all 225 `.text` addresses are now accounted for: 221 unique, 2 by ordinal, 2 hand-resolved, 0 ambiguous, 0 not-found, `hooks.toml` cross-check 25/25, displacements -320..+640 bytes. (The 2026-07-25 figure was 212 of 216, against a harvest that was missing twelve addresses it could not see.) Against **KOTOR 2** (Steam `swkotor2.exe`, the Aspyr 2015 rebuild) **0 of 213 resolved**. That is the tool's boundary, and it is a property of the problem rather than a limitation to be tuned away: signature matching finds *the same compiled bytes relocated*, and KOTOR 2 is a full recompile — different studio, forked source, an 11-year-later toolchain, `.text` 70% larger (0x5846A8 vs 0x33C000). No amount of extra wildcarding recovers a function that was re-emitted rather than moved. So `sigscan` is the right tool for KOTOR 1 variants (Allard, GoG re-packs, future re-releases) and contributes **nothing** to a KOTOR 2 port; that port needs its own address database, populated by RE, with `GameVersion` name lookup as the shared seam. See `docs/translation-additions.md`.
...
- `7` — sigscan: one or more `.text` addresses did not resolve on the target build (the table was still written, but it is incomplete)
