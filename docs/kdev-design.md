...
- **`kdev sigscan [--reference FILE] [--target FILE] [--out DIR] [--emit-only]`** — rebases every hardcoded engine address onto a different build of `swkotor.exe`. Supporting another executable is a *relocation* problem, not a re-RE problem: the functions are byte-identical, the linker just moved them (measured +96..+512 bytes against the Allard Russian exe, varying per function and not monotonic). Two halves: harvest addresses, then locate them.
  - **Harvest** (`EngineAddresses`): the 25 `hooks.toml` detour sites (with their recorded `original_bytes`) plus every `constexpr uintptr_t kAddr… = 0x…;` in `patches/Accessibility/`, with `//` and `/* */` text stripped first — the sources quote addresses in prose constantly and those are not dependencies. Currently 280 references / 264 distinct addresses, 216 of them in `.text`; the rest are `.data`/`.rdata` and need no rebasing (verified unchanged between builds).
  ...
  **Measured applicability (2026-07-25).** Against the Allard Russian exe — a different build of the *same* BioWare source — 212 of 216 `.text` addresses resolved uniquely, `hooks.toml` cross-check 25/25. Against **KOTOR 2** (Steam `swkotor2.exe`, the Aspyr 2015 rebuild) **0 of 213 resolved**. That is the tool's boundary, and it is a property of the problem rather than a limitation to be tuned away: signature matching finds *the same compiled bytes relocated*, and KOTOR 2 is a full recompile — different studio, forked source, an 11-year-later toolchain, `.text` 70% larger (0x5846A8 vs 0x33C000). No amount of extra wildcarding recovers a function that was re-emitted rather than moved. So `sigscan` is the right tool for KOTOR 1 variants (Allard, GoG re-packs, future re-releases) and contributes **nothing** to a KOTOR 2 port; that port needs its own address database, populated by RE, with `GameVersion` name lookup as the shared seam. See `docs/translation-additions.md`.
