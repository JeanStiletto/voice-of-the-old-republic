...
- BTN_Accept id 12 (strref 48379 "Objekt erstellen") commits the SELECTED row: component dispatcher @0x008D2150 branches on the create-vs-breakdown flag [panel+0x3ee4] into create @0x008D4AC0 / breakdown @0x008D4680 (row passed as param). Chemical twin @0x008D7A80, flag at [panel+0x3408], create @0x008D98F0 / breakdown @0x008D95E0. The dispatcher fetches the row with `GetSelectedRow` (0x0041FFB0 = `GetRow(this->selection_index)`; GetRow 0x0041FF70 indexes the same controls array at listbox+0x2ac that a chain walk scans, so a written selection_index IS the engine's index) — but **all four workers open with `if (row->is_active == 0) return;` and return SILENTLY**. Only a real mouse-enter raises that flag (`CSWGuiListBox::SetActiveControl` 0x0041E9A0 → 0x0041E870 fires the row's HandleInputEvent(0) and the old row's HandleInputEvent(1), then stores selection_index), and the chain's cursor warp does not hit-test onto these rows. Writing the index without raising is_active fires Accept into a row the engine considers inactive — the whole "nothing can be crafted" bug (2026-08-13). The create path also gates on the pool (`panel+0x3ec4` vs cost 0x008D6B40 → message box strref 116297) and installs a Yes/No confirm callback @0x008D5FF0 for recipes expensive relative to the skill cache; the direct build is 0x008D5E20.
- The panel opens in CREATE view: Show (vtable slot 18, component 0x008D2D70) ends by calling the create-view switch 0x008D6850, which is the only writer of `flag=1`; 0x008D65D0 writes `flag=0` (breakdown). Which view a given station opens in is chosen by 0x008D6470 off the placeable's byte at +0x296 (1 = create, 2 = breakdown, 3 = both banks shown). BTN_Examine (component id 13 / chemical id 14, strref 48373 "Inventar betrachten") toggles the views @0x008D4F60. BTN_Cancel (component id 27 "Zurück" / chemical id 13 "Schließen") is engine-native Esc (0x62). Medical twins registered at the same ctor positions: 0x008D94D0/0x008D9D60 (accept/examine bank) and 0x008D7A80.
...
- **Prices.** Cost fns are pure functions of the item (their `this` is stored and never read): component 0x008D6B40 = `max(1, GetItemValue/20)`, chemical 0x008DB5E0 = `max(1, GetItemValue/2)`, both over 0x00606A80. Breakdown yield = `max(1, (int)(cost * skillFactor))` with the ctor-cached factor at component +0x3ED4 / chemical +0x33F4 (GetSkillRank(Repair or Treat Injury)/20, clamped to 1.0). **LBL_COST_VALUE / LBL_STOCK_VALUE do NOT track selection_index** — they refresh only on the row's mouse-enter event and on commit/populate, so a keyboard-driven selection leaves them stale (live evidence patch-20260813-132834.log). Call the cost fn; never quote those two labels. LBL_CREDITS_VALUE (the pool) IS reliable.
...
**How to apply:** panel detection is vtable-only (`Kotor2Only(0x009A8B4C)` CreateItem / `(0x009A8CBC)` CreateMedicalItem → PanelKind::WorkbenchCreateItem/WorkbenchCreateMedical). Keyboard commit uses no engine addresses: `menus_crafting::DispatchRowCommit` writes the listbox selection_index to the chain row, raises the row's AND the commit button's `is_active` via `pending::RaiseIsActiveIfZero` (mandatory — see the silent-return gate above), and FireActivates the commit button by .gui id inside one drain tick (Kind::CraftRowCommit). `crafting::SyncSelectedRowFromChainFocus` does the index+flag half per chain step so the panel's own "Objekt erstellen" button acts on the focused row. Chain filtering, view-flip announce and rebind live in menus_crafting.cpp.
...
- **Feats/Powers**: 2D `CSWGuiSkillFlow` charts (field31 +0x3f88 / field30 +0x3f78). The engine DOES nav these natively — forward to `HandleInputEvent(panel, code, 1)` (@0x6ae5f0): 0x31=row-up, 0x32=row-down (runs `SkillFlowChart::HandleInput`@0x6cdd80 + `OnEnterFeat`/`OnEnterPower`). The chart WRAPS, so pre-clamp on chart row (field_0xd) vs row-count (field1_0x4).

**Chart columns are RANKS, and the engine parks the cursor at column 0.** A chart row is one feat/power chain; its up-to-3 columns (`+0x5c + col*0x128`, id at `+0x11c`, `0xffffffff` = empty) are that chain's ranks. Row-stepping never advances the column, so keyboard nav spoke rank I regardless of what was trained. Fix (`SelectMaxRankInRow`): after each row landing, scan the row right-to-left for the first filled cell and drive `SkillFlowChart::SetSelectedSkill`@0x6cdc00 + `OnEnterFeat`/`OnEnterPower` for that id. Do NOT use the engine's own column codes 0x2f/0x30 — every column path in HandleInputEvent calls `PlayGuiSound` first, so walking right would click once per step.

**Highest filled column == highest rank OWNED, on this screen only.** `SetCharacter` builds the feats chart with the view-only `CreateFeatChart(chart, CSWSCreatureStats*)`@0x6ce370 overload, which emits a row only when `HasFeat(root)` and fills a successor column only when `HasFeat(successor)`; `CreatePowerList` calls `CreatePowerChart(.., 0)`@0x6ce0f0, gated the same way on `HasSpell`. Prerequisite rules make gaps impossible. **The per-cell status field is useless here** — `SetCharacter` follows the build with `SetSkillStatusAll(chart, 6)`, stamping every cell identically, unlike the chargen/level-up grids where status carries avail/granted/existing/locked/chosen.

**The three shared label pairs (rank/bonus/total) are re-purposed per tab.** Skills: Fähigkeitenrang / Bonus / Gesamtrang. Powers: **Basispreis** (strref 0x9317) / **`<Helle|Dunkle> Seite` Anp.** (0xa3a2/0xa3a3 + 0xbb10) / **Kosten pro Verwendung** (0xa3b1) — the force-point cost, the alignment adjustment, and the adjusted total, all written by `OnEnterPower` and made visible by UpdateView. Feats: UpdateView CLEARS the visible bit on all six and nothing repaints them, so they hold whichever tab ran last. So: announce stats on Skills and Powers, never on Feats.
