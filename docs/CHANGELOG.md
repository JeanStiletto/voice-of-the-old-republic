...
- The Shift+L level-up shortcut no longer lets you level up endlessly. Pressing Shift+L opened the level-up screen every time, even when your character hadn't earned enough experience for the next level — so you could keep applying level-ups well past where the game should stop. It now respects the same condition the game uses to enable the "Level Up" button on the character screen: when you haven't earned the next level yet (or you're at the level cap) it simply says "Not enough experience to level up yet" and does nothing, and when you genuinely can level up it opens the screen exactly as before.
- Repeated objects now get the same number whether you reach them by target-cycling (Q and E) or by the in-world cycle (comma and period), and that number no longer shifts around as you explore. When several things in an area share a name — three footlockers, a row of identical doors — the mod adds a number so you can tell them apart ("Footlocker 2"). Those numbers used to be worked out two different ways: target-cycling numbered them in the order you happened to look at them, while the comma/period cycle numbered them by position but only counted the ones you'd already found, so discovering another one could renumber the rest. Now both keys use one scheme — fixed, named characters keep an order-met number (since they move about), and everything stationary is numbered north-to-south across the whole area. That means the same footlocker is, say, "Footlocker 3" from either key, every visit and after reloading; the trade-off is that the first one you find might already be "3" if two more sit north of it that you haven't reached yet. Keys 6 and 7 were crossed: pressing 7 announced "Miscellaneous" but actually fired the Explosives column, and 6 did the reverse. Most of the time this just meant the wrong item went off, but it was most visible on the Manaan seabed — pressing 7 to use the sonic emitter (your only usable item there) announced it yet did nothing, while 6 used it without saying so. The keys are now in a straight line — 4 Force Powers, 5 Medical, 6 Miscellaneous, 7 Explosives — and the bare keys, the Shift+number action menus, and the F1 key list all agree with what the game actually does.
- The mod's shortcuts that use Shift, Ctrl or Alt no longer also trigger the game's plain-key action on the same key while you're out in the world. The game ignores modifiers — it reads Shift+4 the same as 4 and Shift+L the same as L — so pressing one of the mod's modified shortcuts used to fire the game's bare-key action as well: opening the action submenu with Shift+4 also quietly queued a Force power, and opening level-up with Shift+L also opened the Quests journal. Now any key pressed with Shift, Ctrl or Alt while you're in the world is reserved for the mod, so the game's shortcut only fires on the unmodified key — bare 4, bare L, bare J and so on still work exactly as before. This also clears the way for letting you reassign the mod's hotkeys freely later. (As part of the same fix, opening an action submenu with Shift and a number no longer occasionally announces a phantom "Heal, slot 0" for an action that never fired.) For now this reservation applies while you're playing in the world; modified keys pressed while a game menu is open aren't covered yet.
