...
- Tolk (LGPL, dynamically loaded) bridges to NVDA, JAWS, SAPI, etc.
...
├── tolk.{h,cpp}          screen-reader bridge (LoadLibrary, lazy init)
...
third_party/tolk/         vendored Tolk x86 binaries (Tolk.dll +
                          nvdaControllerClient32.dll) + LGPL licenses
```

Hooks land mid-function and capture `this` / args from registers (the upstream framework's stack-source path has a known LEA-vs-MOV bug). Tolk is initialized lazily on first hook fire (not in DllMain — Tolk loads COM and driver DLLs, both unsafe under the loader lock).
...
- [Tolk](https://github.com/dkager/tolk) prebuilt x86 binaries (vendored under `third_party/tolk/`).
...
Tolk (vendored under `third_party/tolk/`) is LGPLv3 — see `third_party/tolk/LICENSE.txt` and `LICENSE-NVDA.txt`.
