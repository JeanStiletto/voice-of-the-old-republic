...
#ifdef _WIN32
// SEH-guarded backend initialize(). A backend's initialize() may reach a vendor
// DLL (ZDSR -> ZDSRAPI.dll, PC-Talker -> PCTKUSR.dll, BoyPC -> BoyCtrl.dll).
// When the user's installed DLL exports a mismatched symbol set, the MSVC
// delay-load helper raises a structured exception (0xC06D007F PROC_NOT_FOUND /
// 0xC06D007E MOD_NOT_FOUND) from inside initialize(); unguarded it propagates
// out of *_best() and crashes the host. One broken (typically low-priority)
// backend must not take down backend selection, so we treat a faulting
// initialize() as "failed to initialize" and skip it, letting the walk fall
// through to the next backend (down to SAPI).
//
...
std::shared_ptr<TextToSpeechBackend> FrozenRegistry::create_best() {
  for (auto &e : entries) {
    if (!e.reg.factory)
      continue;
    if (auto b = e.reg.factory(); b && try_initialize(b.get())) {
      return b;
    }
  }
  return nullptr;
